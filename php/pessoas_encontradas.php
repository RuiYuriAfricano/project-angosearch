...
<?php
    ...
    new Traffic();
    $con=conecta();
    $r=$con ->query("select visualizacao from pagina where id_pagina='7' and estado='1' ") or die(mysql_error());
    $row = mysqli_fetch_assoc($r);
    $visualizacao = $row['visualizacao'];
    $visualizacao_mais = $visualizacao+1;
    $atuliza= $con ->query("update pagina set visualizacao='$visualizacao_mais' where id_pagina ='7'") or die("erro ao atualizar visualização");
...
<head>
    ...
    <title>Desaparecidos Encontrados</title>
    <!-- Bootstrap CSS -->
...
telefone1,telefone2,postado_por,data_desaparecimento,dataRegistro,dataEncontrado,bairro,genero,provincia FROM
 ...
 and fk_provincia=id_provincia ORDER  BY  dataExcluido DESC ");
        ...
        <table  style="text-align: center;" width="100%"   id="example1" class="table table-bordered table-striped" >
            <thead > <tr >
                ...
                <th width="96" align="center"></th><th></th>
            ...
            <?php while($dados=mysqli_fetch_array($sql)){ $ft=$dados['foto'];?>
                <tr style="">
                    ...
                    <td width="92" align=""><?php echo $dados['data_desaparecimento'];?></td>
                    <td width="100" align=""id="bt4"><?php  echo date('d/m/Y ',strtotime($dados['dataEncontrado']));
                        ?></td><td></td>


                    <td> <a href="maisEncontrados.php?id_desaparecido=<?php echo ($dados['id_desaparecido']);?> "
                            style="">
