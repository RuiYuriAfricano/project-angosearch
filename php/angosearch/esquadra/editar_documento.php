...
if(isset($_POST['sub'])) {
    ...
    if((mysqli_num_rows($p)) > 0):
        ...
        if ($detalhe == "" and $nomeDoc==""  ) {
            header('Location:view_doc.php?msg=erro');
        } else {
...
fotografia ='$uploadfile' where id_doc='$id' ")
                        ...
                        header('Location:view_doc.php?msg=add');
                    }else{

                        header('Location:view_doc.php?msg=2'); //tamanho grande
                    }

                }else{

                    header('Location:view_doc.php?msg=3'); // extensão da foto é invalida
                }
            }else{
                $up = $con->query("update documentos set nome_doc='$nomeDoc', detalhe='$detalhe',codigo_doc='$codigo_doc'
 WHERE id_doc='$id'")
                or die ("erro ao atualizar documento".mysql_error());
                header('Location:view_doc.php?msg=4');
            }
        ...
        header("Location:view_doc.php?msg=senhaerror");
    endif;
}
...
<head>
    ...
    <title>Editar Documento Perdido</title>
    <!-- Bootstrap CSS -->
    <!-- Required meta tags -->
                        ...
                        <form role="form" method="post" action="" enctype="multipart/form-data">
                            <div class="box-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="detalhe" rows="3" class="form-control" id="nome" pattern="[A-Za-zà-ýÀ-Ý-ç ]*"><?php echo $dados['detalhe']; ?></textarea>
                                </div>
                                <div class="form-group">
                                    <label for="exampleInputFile">Código do Documento</label>
                                    <input type="text" class="form-control" pattern="[A-Za-z0-9!@.\-_]*" value="<?php echo $dados['codigo_doc']; ?>" name="codigo" placeholder="ex: Nº do BI, de Cédula, etc...">
                                </div>
