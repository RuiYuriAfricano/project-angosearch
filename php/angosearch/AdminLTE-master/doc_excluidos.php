...
<head>
    <meta charset="UTF-8">
    <title>Documentos Excluídos | Angosearch</title>
...
margin-bottom: 30px; padding-bottom: 10px; padding-top: 10px; margin-top: 30px; padding-left: 20px;padding-right: 20px;">
            ...
            $docs = $con->query("select * from documentos where estado='2'");
            $totalDocs = mysqli_num_rows($docs);

            $sql=$con->query("SELECT * FROM documentos
WHERE estado = '2'  ORDER  BY  dataExcluido DESC");


            ?>
            <h5 align="left" style="color: #999;"><?php if($totalDocs > 1){
                    echo $totalDocs." Documentos Excluídos";}
                else if($totalDocs == 0){
                    echo "Nenhum Documento Excluído";
                }
                if($totalDocs == 1){
                    echo $totalDocs." Documento Excluído";}
                ?></h5>
            ...
            <table   style="text-align: center;"  width="100%" id="example1" class="table table-bordered table-hover" >
                <thead style="color: #fff;background-color: #3c8dbc"> <tr >
                    ...
                    <th width="192" align="center">Excluído desde</th>
                    <th width="192" align="center">Excluído Por</th>

                    <th></th>

                    <th width="192" align="center"><a href="view_docPerdidos.php">
                            <button class="btn btn-default" style="">Documentos Perdidos</button>
                        </a></th>
